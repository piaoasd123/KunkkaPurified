namespace Heroes
{
    ...
    internal class Kunkka : IHero
    {
        ...
        private KunkkaMenu menuManager;
        ...
        public void OnLoad()
        {
            ...
            heroTeam = hero.Team;
            sleeper = new Sleeper();
            menuManager = new KunkkaMenu(hero.Name);
        ...
        private static bool IsDuringTPSequence;
        private static bool JustArrived;
        private static bool ShouldBuyTP = true;
        private static float ETArrival = 0;
        private static float ETReturn = 0;
        private static Item TownPortalScroll;
        private static Item Bottle;
        private static Item StashBottle;
        private void TPHandler()
        {
            // We always check for bottle because we can still drink shortly after return
            if (Game.RawGameTime < ETReturn + 2)
            {
                Bottle = hero.FindItem("item_bottle");

                StashBottle = hero.Inventory.StashItems.Find(x => x.Name == "item_bottle");
                //Console.WriteLine("StashBottle:  " + StashBottle);

                if (Bottle != null)
                {
                    if (Bottle.Cooldown == 0)
                    {
                        Bottle.UseAbility(hero, true);
                    }
                }

                if (StashBottle != null)
                {
                    StashBottle.UseAbility(hero, true);
                }
            }
            else
            {
            ...
            if (JustArrived)
            {
                if (Game.RawGameTime > ETReturn)
                {
                    IsDuringTPSequence = false;
                    JustArrived = false;
                    ETReturn = 0;
                    ETArrival = 0;
                }

                TownPortalScroll = hero.FindItem("item_travel_boots") ?? hero.FindItem("item_travel_boots_2") ?? hero.FindItem("item_tpscroll");
                // Get some extra TP first
                if (TownPortalScroll == null && ShouldBuyTP)
                {
                    Player.BuyItem(hero, 46);
                    ShouldBuyTP = false;
                }
                else
                {
                    foreach (Item i in hero.Inventory.StashItems.Where(x => x.Name != "item_bottle"))
                    {
                        i.MoveItem((ItemSlot)hero.Inventory.FreeSlots.Last());
                    }
                }
            ...
            {
                if (IsDuringTPSequence)
                {
                    if (Game.RawGameTime > ETArrival)
                    {
                        JustArrived = true;
                    }
                }
                ...
                {
                    if (menuManager.TpHomePressed)
                    {
                        // Check requirements for TP
                        TownPortalScroll = hero.FindItem("item_travel_boots") ?? hero.FindItem("item_travel_boots_2") ?? hero.FindItem("item_tpscroll");
                        if (TownPortalScroll == null)
                        {
                            return;
                        }
                        var manaRequired = TownPortalScroll.ManaCost + hero.Spellbook.SpellE.ManaCost;
                        if (manaRequired > hero.Mana)
                        {
                            return;
                        }
                        //Game.RawGameTime > (ETArrival + 70 - 3) && 
                        if (xMark.Ability.Cooldown == 0)
                        {
                            var fountain = ObjectManager.GetEntities<Unit>().FirstOrDefault(x =>
                                x.Team == heroTeam &&
                            ...
                            xMark.UseAbility(hero);
                            TownPortalScroll.UseAbility(fountain, true);
                            IsDuringTPSequence = true;
                            ETArrival = Game.RawGameTime + xMark.CastPoint + 3;
                            ETReturn = ETArrival + 5 - xMark.CastPoint;
